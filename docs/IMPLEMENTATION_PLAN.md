...
interface Employee {
  ...
  prefix?: string;       // "С" (старший), "Д" (диспетчер), "Н" (начальник)
}
...
interface Brigade {
  ...
  shiftDay: string;      // "8\20", "9\21"
  shiftNight: string;    // "20\8", "21\9"
...
interface Roster {
  ...
  doctorSignature: string; // "Юдаков Е.Ю."
  nurseName: string;     // Фельдшер
  nurseSignature: string;  // "Лазарь Д.Г."
...
interface Settings {
  ...
  doctorName: string;    // Врач СМП (Зав. п\с № 11)
  doctorSignature: string;
  nurseName: string;     // Фельдшер(Старший) п\с № 11
  nurseSignature: string;
  managers: string[];    // Руководители
}
```
...
function generateRoster(date: string, schedule: MonthlySchedule, settings: Settings): Roster {
  ...
  // 2. Сгруппировать по бригадам (БИТ, Пед, Лин, 1170)
...
│                    ВЫЕЗДНЫЕ БРИГАДЫ                             │
├──────────┬──────────────┬───────────┬──────┬───────────────────┤
│Бригада   │Состав (день) │Время      │Подпись│Бригада (ночь)   │
│смена     │              │прихода    │      │смена              │
├──────────┼─────────────────────────┼──────┼───────────────────┤
│БИТ 1110  │С Рыжакова А.А│           │      │БИТ 1110           │
│8\20      │С Сохлов В.А  │           │      │20\8               │
│          │              │           │      │С Рыжакова А.А     │
│          │              │           │      │С Н слабко Л.В.    │
└──────────┴──────────────┴───────────┴──────┴───────────────────┘
```
...
│                    ДИСПЕТЧЕРСКАЯ                                │
├──────────┬──────────────┬───────────┬──────┬───────────────────┤
│смена     │Состав (день) │Время      │Подпись│смена (ночь)     │
├──────────┼──────────────┼───────────┼──────┼───────────────────┤
│8\20      │Д Барауля Т.Л.│           │      │20\8               │
└──────────┴──────────────┴───────────┴──────┴───────────────────┘
